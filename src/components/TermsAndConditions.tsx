import React from 'react';

const TermsAndConditions: React.FC = () => {
  return (
    <section className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-8 py-16">
        <h1 className="text-4xl font-bold text-[#1A1A1A] mb-4">
          Conditions Générales
        </h1>
        <p className="text-gray-600 mb-12">
          Dernière Mise à Jour: 22 Avr 2024
        </p>

        <div className="prose prose-lg max-w-none space-y-8 text-gray-700">
          <p>
            Lorem ipsum dolor sit amet consectetur. Non in convallis rhoncus ipsum et aliquam tellus lectus. 
            Nam convallis tellus ultrices integer urna proin at tempor bibendum. Libero lectus pretium nibh 
            porttitor amet et justo. Erat semper risus elit condimentum ut viverra elit ipsum nisi. Nunc ac 
            augue interdum id sociis enim in. Mattis elit imperdiet tempus adipiscing arcu sapien in. Nulla 
            elementum venenatis in tempus enim volutpat. Lectus sagittis et luctus nulla. Semper dui 
            accumsan at porttitor eget mauris morbi commodo tellus. Habitant at purus orci facilisis 
            scelerisque magna. Venenatis est habitasse molestie mauris eget vitae semper sit non. In dolor 
            elementum commodo nulla duis vel. Neque ultrices elementum consequat pretium magna 
            tortor lobortis et rhoncus.
          </p>

          <p>
            Massa aliquet aenean eu dolor eget at. Laoreet sit egestas vitae risus a ac non iaculis. Pharetra 
            sed sem risus non faucibus rhoncus tortor. Dapibus placerat amet volutpat sed congue in 
            pharetra quis. Rhoncus sit massa eget cursus venenatis nascetur arcu.
            Scelerisque amet tempor ultrices porttitor viverra pretium ullamcorper. Condimentum sem sit.
          </p>

          <p>
            Lorem ipsum dolor sit amet consectetur. Non in convallis rhoncus ipsum et aliquam tellus lectus. 
            Nam convallis tellus ultrices integer urna proin at tempor bibendum. Libero lectus pretium nibh 
            porttitor amet et justo. Erat semper risus elit condimentum ut viverra elit ipsum nisi. Nunc ac 
            augue interdum id sociis enim in. Mattis elit imperdiet tempus adipiscing arcu sapien in. Nulla 
            elementum venenatis in tempus enim volutpat. Lectus sagittis et luctus nulla. Semper dui 
            accumsan at porttitor eget mauris morbi commodo tellus. Habitant at purus orci facilisis 
            scelerisque magna. Venenatis est habitasse molestie mauris eget vitae semper sit non. In dolor 
            elementum commodo nulla duis vel. Neque ultrices elementum consequat pretium magna 
            tortor lobortis et rhoncus.
          </p>

          <p>
            Massa aliquet aenean eu dolor eget at. Laoreet sit egestas vitae risus a ac non iaculis. Pharetra 
            sed sem risus non faucibus rhoncus tortor. Dapibus placerat amet volutpat sed congue in 
            pharetra quis. Rhoncus sit massa eget cursus venenatis nascetur arcu.
            Scelerisque amet tempor ultrices porttitor viverra pretium ullamcorper. Condimentum sem sit.
          </p>
        </div>
      </div>
    </section>
  );
};

export default TermsAndConditions;